...
class admin_welcome_model extends CI_Model
{
	...
	{
		$q=$this->db->select(['id','pname','price','cname','email','mno'])->from('order_booking')->where('type','online')->get();
		return $q->result();
	}
	
	public function search_res($email)
	{
		$q=$this->db->select(['id','pname','price','cname','email','mno','type'])->from('order_booking')->where('email',$email)->get();
		return $q->row();
	}
	public function ofline_order_page()
	{
		$type="ofline";
		$q=$this->db->select(['id','pname','price','cname','email','mno'])->from('order_booking')->where('type',$type)->get();
		return $q->result();
	}
}
